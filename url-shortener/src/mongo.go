...
func NewDB() (*Store, error) {
	logger.Println("[NewDB] Mongo host: ", conf.Mongo.Host)
	clientOptions := options.Client().ApplyURI(conf.Mongo.Host)
	client, err := mongo.Connect(context.TODO(), clientOptions)
	if err != nil {
		logger.Println("[NewDB] Can NOT connect to mongo db: ", err)
		return nil, err
	}
	err = client.Ping(context.TODO(), nil)
	if err != nil {
		logger.Println("[NewDB] Ping mongo failed")
		return nil, err
	}
	logger.Println("[NewDB] Connected to mongo db")
	ret := &Store{c:client}
	return ret, nil
}
...
func (s *Store) GetEntry(shortUrl string) (string, error) {
	...
	err := collection.FindOne(context.TODO(), filter).Decode(&result)
	if err != nil {
		logger.Println("[GetEntry] Find shortened URL failed: ", err)
		return "", errors.New("Find shortened URL failed")
	}
	logger.Println("[GetEntry] Get resutl from mongo: ", result)
